...
if ~exist('reference_file', 'var'), reference_file = 'H:\Dropbox\shared\BGMG\HAPGEN_EUR_100K_11015883_reference_holland.mat'; end;
...
if QQ_PLOT_TRUE
    true_params = struct('sig2_zero', 1, 'pi_vec', trait1_data.causal_pi, 'sig2_beta', trait1_data.sigsq);
    options.plot_HL_bins = plot_HL_bins;
    [figures, plot_data] = UGMG_qq_plot(true_params, trait1_data.zvec, 2 .* mafvec .* (1-mafvec), trait1_data.nvec, pruneidxmat_or_w_ld, ref_ld, options);
    % To reproduce the same curve: plot(plot_data.data_logpvec, plot_data.hv_logp, plot_data.model_logpvec, plot_data.hv_logp)
    result.qq_plot_true.plot_data = plot_data;
    print(figures.tot, sprintf('%s.qq.true.pdf', out_file), '-dpdf')
end
