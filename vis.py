...
figures_folder = drive_letter + r':\Users\oleksanf\Dropbox\analysis\2018_09_24_BGMG_manuscript\figures_and_tables_laptop_2018_10_31'
...
format_string_UGMG_fit  = drive_letter + r":\work\fit_run3_new\{}_noMHC"            +r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run3.fit.json"
format_string_UGMG_fit_censored  = drive_letter + r":\work\LDSR_BGMG_result\{}_noMHC"            +r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run7.fit.json"
format_string_UGMG_test = drive_letter + r":\work\LDSR_BGMG_result\{}_noMHC"        +r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run3.test3.json"
format_string_UGMG_test_censored = drive_letter + r":\work\LDSR_BGMG_result\{}_noMHC"            +r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run7.test.json"
format_string_BGMG_fit  = drive_letter + r":\work\fit_run3_new\{}_noMHC_vs_{}_noMHC"+r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run3b.fit.json"
format_string_BGMG_fit_censored  = drive_letter + r":\work\LDSR_BGMG_result\{}_noMHC_vs_{}_noMHC"+r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run7.fit.json"
format_string_BGMG_test = drive_letter + r":\work\LDSR_BGMG_result\{}_noMHC_vs_{}_noMHC"+r".model=full.r2min=p05.randprune=n64p05.kmax=20000.run3b.test3.json"
folder_SIMU_BGMG_11pifrac       = drive_letter + r':\work\SIMU_BGMG_11pifrac'
folder_SIMU_BGMG_11pifrac_wave2 = drive_letter + r':\work\SIMU_BGMG_11pifrac_wave2'
...
folder_simu_bgmg_paper_examples = drive_letter + r':\work\run_simu_bgmg_paper_examples\final'

folder_SUMSTAT = drive_letter + r':\NORSTORE\MMIL\SUMSTAT'
format_string_LDSR_phase3_h2   = folder_SUMSTAT + r"\LDSR\LDSR_Results\{}_noMHC.phase3.h2.log"
format_string_LDSR_phase3_rg   = folder_SUMSTAT +r"\LDSR\LDSR_Results\{}_noMHC.phase3.rg.log"
format_string_LDSR_h2   = folder_SUMSTAT + r"\LDSR\LDSR_Results\{}_noMHC.h2.log"
format_string_LDSR_rg   = folder_SUMSTAT +r"\LDSR\LDSR_Results\{}_noMHC.rg.log"

nsnps = 11015833;

if True:
    traits4_ordered = ['PGC_SCZ_2014_EUR_qc', 'PGC_BIP_2016_qc', 'SSGAC_EDU_2018_no23andMe', 'GIANT_HEIGHT_2018_UKB']
    traits4 = {'GIANT_HEIGHT_2018_UKB':'Height', 'PGC_BIP_2016_qc':'Bipolar Disorder', 'PGC_SCZ_2014_EUR_qc':'Schizophrenia', 'SSGAC_EDU_2018_no23andMe': 'Educational attainment'}
    traits4_short = {'GIANT_HEIGHT_2018_UKB':'HEIGHT', 'PGC_BIP_2016_qc':'BIP', 'PGC_SCZ_2014_EUR_qc':'SCZ', 'SSGAC_EDU_2018_no23andMe': 'EDU'}
    trait4_index_map = {trait:index for index, trait in enumerate([trait for trait in traits4_ordered])}

if True:
    traits_ordered = ['PGC_SCZ_2014_EUR_qc', 'PGC_BIP_2016_qc','PGC_MDD_2018_no23andMe',
                      'PGC_ASD_2017_iPSYCH','PGC_ADHD_2017_EUR','SSGAC_EDU_2018_no23andMe',
                      'GIANT_HEIGHT_2018_UKB']
    traits_short = {'PGC_BIP_2016_qc':'BIP','PGC_SCZ_2014_EUR_qc':'SCZ','PGC_MDD_2018_no23andMe':'MDD',
    ...
    traits = {'GIANT_HEIGHT_2018_UKB':'Height',
              'PGC_BIP_2016_qc':'Bipolar Disorder',
              'PGC_SCZ_2014_EUR_qc':'Schizophrenia',
...
if False:  # other version of traits (SCZ, MDD, EDU and HEIGHT) -- haven't tried so far...
    traits_ordered = ['PGC_SCZ_0917_noMHC', 'PGC_BIP_2016_qc','PGC_MDD_2018_Howard_no23andMe_noUKBB_lift_noMHC',
                      'PGC_ASD_2017_iPSYCH','PGC_ADHD_2017_EUR','SSGAC_EDU_2016_noMHC',
                      'GIANT_HEIGHT_2014_lift_noMHC']
    traits_short = {'PGC_BIP_2016_qc':'BIP','PGC_SCZ_0917_noMHC':'SCZ','PGC_MDD_2018_Howard_no23andMe_noUKBB_lift_noMHC':'MDD',
                      'PGC_ASD_2017_iPSYCH':'ASD','PGC_ADHD_2017_EUR':'ADHD','SSGAC_EDU_2016_noMHC':'EDU',
                      'GIANT_HEIGHT_2014_lift_noMHC':'HEIGHT'}
    traits = {'GIANT_HEIGHT_2014_lift_noMHC':'Height',
              'PGC_BIP_2016_qc':'Bipolar Disorder',
              'PGC_SCZ_0917_noMHC':'Schizophrenia',
              'SSGAC_EDU_2018_no23andMe':'SSGAC_EDU_2016_noMHC',
              'PGC_MDD_2018_Howard_no23andMe_noUKBB_lift_noMHC':'Major Depressive Disorder',
              'PGC_ASD_2017_iPSYCH':'Autism Spectrum Disorder',
              'PGC_ADHD_2017_EUR':'ADHD'}  # Attention deficit hyperactivity disorder
    trait_index_map = {trait:index for index, trait in enumerate([trait for trait in traits_ordered])}
...
# Figures
DO_QQ_MODEL_DATA_vs_NULL = False
DO_QQ_MODEL_vs_DATA = False
DO_QQ_BINS_MODEL_DATA_vs_NULL = False
DO_POWER_PLOT = False
DO_ADJ_TRAJECTORY = False
DO_ADJ_TRAJECTORY_BGMG = False
DO_VENN_DIAGRAMS = False  # works only after DO_BGMG_TABLE
DO_VENN_DIAGRAMS_SUPPL = False
DO_STRATIFIED_QQ=False
...
DO_SIMU_UGMG_TABLE = False
DO_SIMU_BGMG_TABLE = False
...
def make_qq_plot(qq, ci=True):
    hv_logp = np.array(qq['hv_logp'])
    data_logpvec = np.array(qq['data_logpvec'])
    model_logpvec = np.array(qq['model_logpvec'])
...
for censored in [False]:
    if DO_QQ_MODEL_DATA_vs_NULL:
        fig = plt.figure(figsize=(12, 9+9), dpi=80)
        for index, trait in enumerate(traits_ordered):
            plt.subplot(2+2,2,1+index)
            ...
            plt.title(traits[trait])
            #plt.legend(['data', 'model', 'null'], loc='lower right')
            plt.xlabel('Expected(-$log_{10}$ p-value)')
            plt.ylabel('Observed(-$log_{10}$ p-value)')
        fig.subplots_adjust(hspace=0.55)
        savefig(os.path.join(figures_folder, 'QQ_MODEL_DATA_vs_NULL'+(' (censored)' if censored else '')+'.svg'))
        savefig(os.path.join(figures_folder, 'QQ_MODEL_DATA_vs_NULL'+(' (censored)' if censored else '')+'.png'))
    ...
    if DO_QQ_MODEL_vs_DATA:
        ...
        savefig(os.path.join(figures_folder, 'QQ_MODEL_vs_DATA'+(' (censored)' if censored else '')+'.svg'))
        savefig(os.path.join(figures_folder, 'QQ_MODEL_vs_DATA'+(' (censored)' if censored else '')+'.png'))

if DO_QQ_BINS_MODEL_DATA_vs_NULL:
    for index, trait in enumerate(traits_ordered):
        fig = plt.figure(figsize=(16, 12), dpi=80)
        fig.suptitle(traits[trait]) #, fontsize=16)
        ...
        savefig(os.path.join(figures_folder, 'QQ_BINS_MODEL_DATA_vs_NULL_{}.svg'.format(trait)))
        savefig(os.path.join(figures_folder, 'QQ_BINS_MODEL_DATA_vs_NULL_{}.png'.format(trait)))
...
if DO_POWER_PLOT:
    ...
    for index, trait in enumerate(traits_ordered):
        ...
        leg_labels.append('{0} ({1:.1f}%)'.format(traits_short[trait],100* current_s[-1]))
    ...
    savefig(os.path.join(figures_folder, 'POWER_PLOT.svg'))
    savefig(os.path.join(figures_folder, 'POWER_PLOT.png'))
...
if DO_ADJ_TRAJECTORY:
    ...
    savefig(os.path.join(figures_folder, 'ADJ_TRAJECTORY.svg'))
    savefig(os.path.join(figures_folder, 'ADJ_TRAJECTORY.png'))
...
if DO_UGMG_TABLE:
    ...
    for censored in [False, True]:
        for index, trait in enumerate(traits_ordered):
            ...
            insert_key_to_dictionary_as_list('trait', traits[trait] + (' (censored)' if censored else ''))
            for key in ci:
        ...
        for c in [c for c in df.columns if c.startswith('pi_')]: df[c.replace('pi_', 'nc@p9_').replace('_vec', '')] = ['{:.1f}'.format(x*NCKoef/1000) for x in df[c].astype(float).values*nsnps]
        for c in [c for c in df.columns if c.startswith('pi_')]: df[c.replace('pi_', 'nc_'    ).replace('_vec', '')] = ['{:.1f}'.format(x/1000) for x in df[c].astype(float).values*nsnps]
...
if DO_BGMG_TABLE:
    ...
    for censored in [False, True]:
        ...
        for trait1, trait2 in list(itertools.combinations(traits_ordered, 2)):
            ...
            insert_key_to_dictionary_as_list('trait1', traits[trait1] +(' (censored)' if censored else ''))
            insert_key_to_dictionary_as_list('trait2', traits[trait2] +(' (censored)' if censored else ''))
            ...
            for key in keys_pivec:
                insert_key_to_dictionary_as_list(keys_pivec[key], formats[key].format(ci[key]['point_estimate']*nsnps))
                insert_key_to_dictionary_as_list(keys_pivec[key] + '_se', formats[key].format(ci[key]['se']*nsnps))
                insert_key_to_dictionary_as_list(keys_pivec[key]+'@p9', '{:.2f}'.format(ci[key]['point_estimate']*nsnps*NCKoef))
                insert_key_to_dictionary_as_list(keys_pivec[key]+'@p9_se', '{:.2f}'.format(ci[key]['se']*nsnps*NCKoef))
            ...
            for ldsr_formatter, LDSRtag in [(format_string_LDSR_rg, 'LDSR(2)_'), (format_string_LDSR_phase3_rg, 'LDSR_')]:
                for line in open(ldsr_formatter.format(trait1)).read().split('\n'):
                    if '--rg' in line: continue
                    if 'Reading summary statistics from' in line: continue
                    if trait2 not in line: continue
        ...
        bgmg_table_df.loc[(bgmg_table_df.trait2_ID == 'PGC_MDD_2018_no23andMe') & (bgmg_table_df.trait1_ID=='PGC_SCZ_2014_EUR_qc'),'rho12_se']=0
        bgmg_table_df.loc[(bgmg_table_df.trait2_ID == 'PGC_MDD_2018_no23andMe') & (bgmg_table_df.trait1_ID=='PGC_SCZ_2014_EUR_qc'),'rg_se']=0
...
if DO_VENN_DIAGRAMS:
    for censored in [False, True]:
        ...
        plt.tight_layout()
        savefig(os.path.join(figures_folder, 'VENN_DIAGRAMS'+(' (censored)' if censored else '')+'.svg'))
        savefig(os.path.join(figures_folder, 'VENN_DIAGRAMS'+(' (censored)' if censored else '')+'.png'))

        plt.figure(figsize=(30, 28), dpi=80)
        cm = plt.cm.get_cmap('Vega10')
        for index, row in bgmg_table_df.iterrows():
            ...
            for i in range(1,3):
                if i1>i2:
                    ...
                    v = venn2(subsets = (n1, n2, n12), set_labels = (traits_short[t1], traits_short[t2]))
        ...
        plt.tight_layout()
        savefig(os.path.join(figures_folder, 'VENN_DIAGRAMS_sym'+(' (censored)' if censored else '')+'.svg'))
        savefig(os.path.join(figures_folder, 'VENN_DIAGRAMS_sym'+(' (censored)' if censored else '')+'.png'))
...
if DO_VENN_DIAGRAMS_SUPPL:
    ...
    bgmg_table_df = bgmg_table_df_censored[censored]
    max_size = 17
    for trait1_orig in traits_ordered:
        fig=plt.figure(figsize=(16, 28), dpi=80)
        for index, trait2_orig in enumerate([x for x in traits_ordered if x != trait1_orig]):
            ...
            plt.subplot(len(trait_index_map)-1, 3, 3*index + 1)
            v = venn2(subsets = (n1, n2, n12), normalize_to=(n1+n2+n12)/max_size, set_labels = ("", ""))
            v.get_patch_by_id('100').set_color(cm.colors[f(trait_index_map[t1])])
            v.get_patch_by_id('010').set_color(cm.colors[f(trait_index_map[t2])])    
            v.get_patch_by_id('110').set_color(cm.colors[7])   
            ...
            plt.xlim([-0.75, 0.75]), plt.ylim([-0.6, 0.6])
            if flip_data: plt.title(traits_short[row.trait2_ID] +' & ' + traits_short[row.trait1_ID])
            else: plt.title(traits_short[row.trait1_ID] +' & ' + traits_short[row.trait2_ID])

            fname1 = format_string_BGMG_test.format(trait1, trait2)
            fname2 = format_string_BGMG_test.format(trait2, trait1)
            if os.path.exists(fname2):
                data = json.loads(open(fname2).read())
                flip_data = True
            elif os.path.exists(fname1):
            ...
            for repeat in range(0,2):
                plt.subplot(len(trait_index_map)-1, 3, 3*index + 2 + ((1-repeat) if flip_data else repeat))
                ...
                hNull = plt.plot(qq['hv_logp'], qq['hv_logp'], 'k--')
                plt.legend(['All SNPs'] + '$P{0}\leq0.1$ $P{0}\leq0.01$ $P{0}\leq0.001$'.format('_{' + traits_short[trait2]+'}').split(), loc='lower right', fontsize=14, borderpad=0.2, frameon=False, borderaxespad=0.2, labelspacing=0.1)
                for index2, qq in enumerate(sqq['trait2' if repeat==0 else 'trait1']):
                    hModel = plt.plot(qq['model_logpvec'], qq['hv_logp'], color=cm.colors[index2], linestyle='dashed')
                plt.ylim(0, 7.3); plt.xlim(0, 7.3)
                if flip_data: plt.title('{} | {}'.format(traits_short[trait2], traits_short[trait1]))
                else: plt.title('{} | {}'.format(traits_short[trait1], traits_short[trait2]))
                trait1, trait2 = trait2, trait1  # swap traits
        fig.subplots_adjust(hspace=0.25, wspace=0.35)

        savefig(os.path.join(figures_folder, 'SF_VENN_and_stratQQ_{}.svg'.format(traits_short[trait1])))
        savefig(os.path.join(figures_folder, 'SF_VENN_and_stratQQ_{}.png'.format(traits_short[trait1])))
...
if DO_ADJ_TRAJECTORY_BGMG:
    ...
    savefig(os.path.join(figures_folder, 'ADJ_TRAJECTORY_BGMG.svg'))
    savefig(os.path.join(figures_folder, 'ADJ_TRAJECTORY_BGMG.png'))
...
if DO_STRATIFIED_QQ:
    ...
    savefig(os.path.join(figures_folder, 'STRATIFIED_QQ.svg'))
    savefig(os.path.join(figures_folder, 'STRATIFIED_QQ.png'))
...
if DO_SIMU_QQ:
    ...
    savefig(os.path.join(figures_folder, 'SIMU_QQ.svg'))
    savefig(os.path.join(figures_folder, 'SIMU_QQ.png'))
...
if DO_SIMU_QQ_BINS:
    for pi_index, pi in enumerate(['3.0000e-03', '3.0000e-04']):
        ...
        fig.subplots_adjust(hspace=0.25, wspace=0.25)
        savefig(os.path.join(figures_folder, 'SIMU_QQ_BINS_{}.svg'.format(pi)))
        savefig(os.path.join(figures_folder, 'SIMU_QQ_BINS_{}.png'.format(pi)))     
...
if DO_SIMU_STRATIFIED_QQ:
    ...
    fig.subplots_adjust(hspace=0.25, wspace=0.25)
    savefig(os.path.join(figures_folder, 'SIMU_STRATIFIED_QQ.svg'))
    savefig(os.path.join(figures_folder, 'SIMU_STRATIFIED_QQ.png'))
...
if DO_SIMU_UGMG:
    ...
    files = glob.glob(folder_SIMU_BGMG_11pifrac+r'\*bgmg.short.json')
    if DO_GCTA: files = glob.glob(folder_SIMU_BGMG_GCTA+r'\*.trait*.fit.short.json')
    for fname in files:
        data = json.loads(open(fname).read())
        for trait_data in data['result']['univariate']:
    ...
    df_agg = df[['true_pi1u', 'true_h2', 'pi_vec', 'pi_vec_se', 'h2', 'h2_se']].groupby(['true_pi1u', 'true_h2']).agg({'pi_vec':['mean', 'std'], 'pi_vec_se':['mean'], 'h2':['mean', 'std'], 'h2_se':['mean']})
    ...
    h2_pi_vec = [('0.1', '3.0000e-03'), ('0.4', '3.0000e-03'), ('0.7', '3.0000e-03'), ('0.1', '3.0000e-04'), ('0.4', '3.0000e-04'), ('0.7', '3.0000e-04')]
    for index in range(0, 2):
        ...
        plt.xticks([r + barWidth for r in range(len(bars1))], ['h2={}\n$\pi_1$={:.0e}'.format(h2, float(pi)).replace('e-0', 'e-') for h2, pi in h2_pi_vec])
        plt.ylabel('h2' if index==0 else '$\pi_1$' )
        plt.legend()
        if index==1: plt.ylim([1e-4, 1e-2])

        fig.subplots_adjust(hspace=0.35)
        savefig(os.path.join(figures_folder, 'SIMU_UGMG_pi_and_h2.svg'))
        savefig(os.path.join(figures_folder, 'SIMU_UGMG_pi_and_h2.png'))
...
if DO_SIMU_BGMG or DO_SIMU_BGMG_TABLE:
    ...
    files = glob.glob(folder_SIMU_BGMG_11pifrac + r'\*run1.bgmg.short.json') + glob.glob(folder_SIMU_BGMG_11pifrac_wave2 + r'\*run1.bgmg.short.json')
    ...
    df_agg = df.groupby(['true_pi1u', 'true_pi2u', 'true_pi12' , 'true_h2', 'true_rg']).agg({'pi12':['mean', 'std'], 'pi12_se':['mean'], 'rg':['mean', 'std'], 'rg_se':['mean'], 'rho12':['mean', 'std'], 'rho12_se':['mean']})
    df_agg.columns = ['_'.join(col).strip() for col in df_agg.columns.values]
    df_agg = df_agg.reset_index()
    for col in ['true_pi1u','true_pi2u','true_pi12','true_h2']: df_agg[col] = df_agg[col].astype(float)
    ...
    if DO_SIMU_BGMG:
        ...
        savefig(os.path.join(figures_folder, '{}_pi12.svg'.format('SIMU_BGMG_old_style', true_rg)))
        savefig(os.path.join(figures_folder, '{}_pi12.png'.format('SIMU_BGMG_old_style', true_rg)))

    for true_rg in ['0.0', '0.5']:
        if not DO_SIMU_BGMG: break
        fig = plt.figure(figsize=(18, 16), dpi=80)
        for index, (pi1u, pi2u) in enumerate([(3e-03, 3e-03), (3e-4, 3e-04), (3e-03, 3e-04)]):
            for jndex, h2_index in enumerate([0.1, 0.4, 0.7]):
                df_plot = df_agg[(df_agg.true_rg==true_rg) & (df_agg.true_pi1u == pi1u) & (df_agg.true_pi2u == pi2u) & (df_agg.true_h2 == h2_index)].sort_values('true_pi12')
        ...
        savefig(os.path.join(figures_folder, '{}_pi12_rg={}.svg'.format('SIMU_BGMG', true_rg)))
        savefig(os.path.join(figures_folder, '{}_pi12_rg={}.png'.format('SIMU_BGMG', true_rg)))
    ...
    for do_rg in [True, False]:  # rg or rho_beta?
        ...
        for true_rg in ['0.0', '0.5']:
            ...
            fig.subplots_adjust(hspace=0.30, wspace=0.50)                    
            savefig(os.path.join(figures_folder, '{}_{}_rg={}.svg'.format('SIMU_BGMG', 'rg' if do_rg else 'rho12', true_rg)))
            savefig(os.path.join(figures_folder, '{}_{}_rg={}.png'.format('SIMU_BGMG', 'rg' if do_rg else 'rho12', true_rg)))
    ...
    if True:
        ...
        fig.subplots_adjust(hspace=0.30, wspace=0.50)                    
        savefig(os.path.join(figures_folder, 'SIMU_BGMG_main_figure2.svg'))
        savefig(os.path.join(figures_folder, 'SIMU_BGMG_main_figure2.png'))

    if DO_SIMU_BGMG_TABLE:
        df_agg.to_csv(os.path.join(tables_folder, 'SIMU_BGMG_TABLE.csv'), sep='\t',index=False)
        if tables_writer is None: tables_writer = pd.ExcelWriter(os.path.join(tables_folder, 'all_tables.xlsx'))
        df=df_agg.copy()
...
if DO_SIMU_UGMG_ANNOTENRICH:
    for mafSuffix in ['bgmgMAF']: # ['']
        ...
        for index in range(0, 2):
            ...
            fig.subplots_adjust(hspace=0.35)
            savefig(os.path.join(figures_folder, 'SIMU_UGMG_ANNOTENRICH{}_pi_and_h2.svg'.format(mafSuffix)))
            savefig(os.path.join(figures_folder, 'SIMU_UGMG_ANNOTENRICH{}_pi_and_h2.png'.format(mafSuffix)))
...
for mafSuffix in ['_bgmgMAF', '']:
    ...
    if DO_SIMU_BGMG_ANNOTENRICH_TABLE:
        ...
        df_agg['N12 (x1000)'] = ['{:.2f} ({:.2f}/{:.2f})'.format(v*koef*nsnps,se*koef*nsnps,sd*koef*nsnps) for v,se,sd in zip(df_agg['pi12_mean'],df_agg['pi12_se_mean'],df_agg['pi12_std'])]
        df_agg['N1u (x1000)'] = ['{:.2f} ({:.2f}/{:.2f})'.format(v*koef*nsnps,se*koef*nsnps,sd*koef*nsnps) for v,se,sd in zip(df_agg['pi1u_mean'],df_agg['pi1u_se_mean'],df_agg['pi1u_std'])]
...
if DO_SIMU_UGMG_SUBREF:
    ...
    df['hat_ncausal'] = nsnps*df['frac'].values*df['pi_vec'].values
    df['hat_ncausal_se'] = nsnps*df['frac'].values*df['pi_vec_se'].values
    df['true_ncausal'] = nsnps*df['true_pi1u']
